...
Install dependencies with `npm ci`. Before submitting, run `npm run check` and `npm test`. Run the app with `npm start` and check the affected flows, keyboard controls and mobile layout. Automated tests mock the provider and use no real key.
