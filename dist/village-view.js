...
export function createView(canvas,world,onSelect){
 ...
 for(const v of world.villagers){const g=new T.Group();scene.add(g);cyl(.27,.32,.7,v.color,0,.8,0,g);ball(.28,0xe7bd95,0,1.4,0,g);const hair=ball(.29,v.id==='pip'?0xc69b62:0x75553e,0,1.52,-.04,g);hair.scale.y=.65;const legs=[],arms=[];for(const dx of [-.14,.14]){const leg=box(.15,.38,.17,0x596157,dx,.26,0,g);legs.push(leg);box(.18,.12,.28,0x614f41,dx,.08,.04,g);const arm=box(.12,.52,.15,v.color,dx*2.5,.87,0,g);arms.push(arm);}for(const dx of [-.095,.095])ball(.032,0x45473b,dx,1.43,.255,g);if(v.id==='mira'){cyl(.36,.38,.08,0xe0bc73,0,1.7,0,g);cyl(.22,.27,.17,0xe0bc73,0,1.81,0,g);}g.traverse(o=>{if(o.isMesh){o.userData.villager=v.id;pickables.push(o);}});const tag=label(v.name,0,2.35,0);const prop=box(.14,.55,.14,0xa58a60,.38,.68,.2,g);prop.visible=false;people.set(v.id,{g,legs,arms,tag,prop});}
 ...
 const rainy=world.weather==='rainy',hour=world.time%1440/60;
 ...
 for(const v of world.villagers){const o=people.get(v.id);o.g.position.set(v.x,0,v.z);if(v.path.length){const p=v.path[0];o.g.rotation.y=Math.atan2(p.x-v.x,p.z-v.z);}o.legs.forEach((l,i)=>l.rotation.x=running&&v.status==='walking'?Math.sin(time*9+i*Math.PI)*.4:0);const acting=v.status==='acting';o.g.position.y=running&&acting?Math.sin(time*3)*.035:0;o.g.rotation.z=acting&&v.action==='rest'?.35:0;o.arms.forEach((arm,i)=>{arm.rotation.x=running&&acting?(v.action==='garden'||v.action==='work'?Math.sin(time*5+i)*.7:v.action==='eat'?-.8:Math.sin(time*2+i)*.25):0;});o.prop.visible=acting&&(v.action==='work'||v.action==='garden');o.tag.position.set(v.x,2.3,v.z);o.tag.material.opacity=v.id===selected?1:.72;}const v=world.villagers.find(v=>v.id===selected);ring.position.set(v.x,.065,v.z);renderer.render(scene,camera);},dispose(){observer.disconnect();renderer.dispose();}};
}
